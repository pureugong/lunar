...
├── dist/                   # Production build output (committed, served by GitHub Pages)
├── src/
...
- **Committed build artifacts**: `dist/` is committed to the repo for GitHub Pages deployment. After changing source code, run `npm run build` and commit the updated `dist/` output.

## Important Notes for AI Assistants

- The `dist/` directory contains minified production output — do not edit it directly. Edit source files in `src/` and rebuild.
